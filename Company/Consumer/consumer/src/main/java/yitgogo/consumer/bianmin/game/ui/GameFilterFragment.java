...
public class GameFilterFragment extends BaseNetworkFragment {
    ...
    private void search() {
        if (TextUtils.isEmpty(game.getId())) {
            ApplicationTool.showToast("请选择游戏名称");
        } else {
    ...
    private void getGames() {
        ...
        post(API.API_BIANMIN_GAME_LIST, new OnNetworkListener() {
            @Override
    ...
    private void getGameArea() {
        ...
        requestParams.add(new RequestParam("gameid", game.getId()));
        post(API.API_BIANMIN_GAME_AREA, requestParams, new OnNetworkListener() {
            @Override
            public void onSuccess(String result) {
                if (!TextUtils.isEmpty(result)) {
                    try {
    ...
    private void getGameServer() {
        ...
        List<RequestParam> requestParams = new ArrayList<>();
        requestParams.add(new RequestParam("gameid", game.getId()));
        requestParams.add(new RequestParam("area", gameArea.getArea()));
        post(API.API_BIANMIN_GAME_SERVER, requestParams, new OnNetworkListener() {
            @Override
            public void onSuccess(String result) {
                if (!TextUtils.isEmpty(result)) {
                    try {
