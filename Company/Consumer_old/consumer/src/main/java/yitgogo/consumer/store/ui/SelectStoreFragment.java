...
public class SelectStoreFragment extends BaseNotifyFragment {
    ...
    class AreaGridAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ViewHolder holder;
            if (convertView == null) {
                convertView = layoutInflater.inflate(R.layout.list_area_selection, null);
                holder = new ViewHolder();
                holder.textView = (TextView) convertView.findViewById(R.id.list_area_selection);
                convertView.setTag(holder);
            } else {
                holder = (ViewHolder) convertView.getTag();
            }
            ...
            if (result.length() > 0) {
                ...
                try {
                    ...
                    if (object.optString("state").equalsIgnoreCase("SUCCESS")) {
                        ...
                        if (array != null) {
                            for (int i = 0; i < array.length(); i++) {
                                stores.add(new ModelStoreSelected(array.getJSONObject(i)));
                            }
                            if (stores.size() > 0) {
                                storeAdapter.notifyDataSetChanged();
                            } else {
